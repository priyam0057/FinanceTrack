...
function FrontendEditorInner({ projectId, isFullscreen, onToggleFullscreen }: FrontendEditorProps) {
  ...
  useEffect(() => {
    if (!initialized) {
      loadDiagram('frontend').then((data) => {
        if (data) {
  ...
  useEffect(() => {
    if (autoSave && initialized) {
      const timer = setTimeout(() => {
        saveDiagram('frontend', nodes, edges);
      }, 2000);
  ...
  const handleAddNode = useCallback((type: CustomNodeType) => {
    setNodes((nds) => [...nds, {
      id: `${type}-${Date.now()}`, type: 'frontend', draggable: true,
  ...
  return (
    <div ref={flowRef} className="h-[600px] w-full rounded-xl border border-border overflow-hidden relative">
      <ReactFlow
        ...
        defaultEdgeOptions={{ style: { strokeWidth: 2, stroke: '#a855f7' }, markerEnd: { type: MarkerType.ArrowClosed } }}
        className="bg-background"
      >
        <DiagramToolbar
          mode="frontend" onAddNode={handleAddNode}
        ...
        <Panel position="top-right" className="flex gap-2">
          <DiagramExport nodes={nodes} edges={edges} projectId={projectId} diagramType="frontend" flowRef={flowRef} onImport={handleImport} />
          <DiagramTemplates nodes={nodes} edges={edges} diagramType="frontend" onLoadTemplate={handleLoadTemplate} />
          <VersionHistory nodes={nodes} edges={edges} projectId={projectId} diagramType="frontend" onRestore={handleLoadTemplate} />
        </Panel>
...
export function FrontendEditor(props: FrontendEditorProps) {
  return <ReactFlowProvider><FrontendEditorInner {...props} /></ReactFlowProvider>;
}
