...
export default function Transactions() {
  ...
  const handleUpdate = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    updateMutation.mutate({
      ...
      date_time: (editingDateTime || new Date(editingTransaction?.date_time)).toISOString(),
    });
  };
                            ...
                            <Dialog>
                              <DialogTrigger asChild>
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() => { setEditingTransaction(transaction); setEditingDateTime(new Date(transaction.date_time)); setEditingPaymentMethod(transaction.payment_method || "cash"); }}
                                >
                              ...
                              <DialogContent className="w-[98vw] sm:max-w-[1600px]">
                                ...
                                <form onSubmit={handleUpdate}>
                                  <div className="grid grid-cols-1 lg:grid-cols-[360px_minmax(420px,1fr)_260px] gap-y-6 gap-x-6">
                                    <div className="space-y-4 bg-card rounded-lg p-4 border">
                                      ...
                                      <div>
                                        ...
                                        <ToggleGroup type="single" value={editingPaymentMethod} onValueChange={(v) => v && setEditingPaymentMethod(v)} className="flex flex-wrap gap-2">
                                        ...
                                        <input type="hidden" name="payment_method" value={editingPaymentMethod} />
                                      </div>
                                      <div>
                                        <Label>Note</Label>
                                        <Textarea name="note" defaultValue={transaction.note} rows={3} />
                                      </div>
                                    </div>

                                    <div className="bg-card rounded-lg p-4 border">
                                      <DateTimePicker value={editingDateTime || new Date(transaction.date_time)} onChange={(d) => setEditingDateTime(d)} showDate={true} showTime={false} />
                                    </div>
                                    ...
                                    <div className="space-y-6 bg-card rounded-lg p-4 border">
                                      ...
                                      <div>
                                        <Label>Time</Label>
                                        <TimePicker value={editingDateTime || new Date(transaction.date_time)} onChange={(d) => setEditingDateTime(d)} />
                                      </div>
                                    </div>
                                  </div>
